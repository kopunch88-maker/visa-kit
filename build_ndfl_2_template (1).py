...
def _make_income_subtable(parent_cell, col_index_offset: int) -> None:
    """Строит одну из двух подтаблиц §3 (5 столбцов × 15 строк = шапка + 14 данных).

    col_index_offset = 0 для левой таблицы (слоты 0..13), 14 для правой (14..27).
    """
    # Убираем дефолтный параграф в ячейке-родителе чтобы таблица не висела с отступом
    parent_cell.text = ""

    inner = parent_cell.add_table(rows=1 + NDFL_3_ROWS_PER_COL, cols=5)
    inner.autofit = False

    # Ширины колонок: внутренняя половина ≈ 9.5 cm
    # Месяц | Код дохода | Сумма дохода | Код вычета | Сумма вычета
    widths_cm = [1.3, 1.4, 2.7, 1.4, 2.7]  # ≈ 9.5
    for i, w in enumerate(widths_cm):
        inner.columns[i].width = Cm(w)

    # Шапка
    headers = ["Месяц", "Код дохода", "Сумма дохода", "Код вычета", "Сумма вычета"]
    for i, h in enumerate(headers):
        _cell_text(
            inner.rows[0].cells[i], h,
            size=FONT_SIZE_SMALL_PT, bold=True,
            align=WD_ALIGN_PARAGRAPH.CENTER,
        )

    # Строки данных
    for r in range(NDFL_3_ROWS_PER_COL):
        slot = col_index_offset + r  # 0..27
        cells = inner.rows[r + 1].cells
        _cell_text(cells[0], f"{{{{ ndfl_2.rows[{slot}].month }}}}",
                   align=WD_ALIGN_PARAGRAPH.CENTER)
        _cell_text(cells[1], f"{{{{ ndfl_2.rows[{slot}].income_code }}}}",
                   align=WD_ALIGN_PARAGRAPH.CENTER)
        _cell_text(cells[2], f"{{{{ ndfl_2.rows[{slot}].income_amount }}}}",
                   align=WD_ALIGN_PARAGRAPH.RIGHT)
        _cell_text(cells[3], f"{{{{ ndfl_2.rows[{slot}].deduction_code }}}}",
                   align=WD_ALIGN_PARAGRAPH.CENTER)
        _cell_text(cells[4], f"{{{{ ndfl_2.rows[{slot}].deduction_amount }}}}",
                   align=WD_ALIGN_PARAGRAPH.RIGHT)

    _set_table_borders_all(inner)
...
def build_template(output_path: Path) -> None:
    ...
    _set_run(r2, size=FONT_SIZE_PT, bold=True)
    ...
    _set_run(r, size=FONT_SIZE_PT, bold=True)
    ...
    for i, h in enumerate(["Фамилия", "Имя", "Отчество*"]):
        _cell_text(t2.rows[0].cells[i], h, size=FONT_SIZE_SMALL_PT,
                   align=WD_ALIGN_PARAGRAPH.LEFT)
    _cell_text(t2.rows[1].cells[0], "{{ applicant.last_name_native }}", bold=True)
    _cell_text(t2.rows[1].cells[1], "{{ applicant.first_name_native }}", bold=True)
    _cell_text(t2.rows[1].cells[2], "{{ applicant.middle_name_native }}", bold=True)
    ...
    for i, h in enumerate(["Статус налогоплательщика", "Дата рождения", "Гражданство (код страны)"]):
        _cell_text(t3.rows[0].cells[i], h, size=FONT_SIZE_SMALL_PT,
                   align=WD_ALIGN_PARAGRAPH.LEFT)
    _cell_text(t3.rows[1].cells[0], "{{ ndfl_2.taxpayer_status }}", bold=True)
    _cell_text(t3.rows[1].cells[1], "{{ ndfl_2.birth_date_str }}", bold=True)
    _cell_text(t3.rows[1].cells[2], "{{ ndfl_2.country_code }}", bold=True)
    ...
    for i, h in enumerate(["Код документа, удостоверяющего личность", "Серия и номер документа"]):
        _cell_text(t4.rows[0].cells[i], h, size=FONT_SIZE_SMALL_PT,
                   align=WD_ALIGN_PARAGRAPH.LEFT)
    _cell_text(t4.rows[1].cells[0], "{{ ndfl_2.id_doc_code }}", bold=True)
    _cell_text(t4.rows[1].cells[1], "{{ ndfl_2.passport_series_number }}", bold=True)
    ...
    # Внешняя таблица-контейнер 1×2 без границ — чтобы две подтаблицы
    # встали бок о бок (python-docx не позволяет иначе)
    t_container = doc.add_table(rows=1, cols=2)
    t_container.autofit = False
    t_container.columns[0].width = Cm(9.5)
    t_container.columns[1].width = Cm(9.5)
    _set_table_no_borders(t_container)

    # Левая подтаблица — слоты 0..13
    _make_income_subtable(t_container.rows[0].cells[0], col_index_offset=0)
    # Правая подтаблица — слоты 14..27
    _make_income_subtable(t_container.rows[0].cells[1], col_index_offset=NDFL_3_ROWS_PER_COL)
    ...
    t5.autofit = False
    for i, w in enumerate([Cm(6.5), Cm(3.0), Cm(6.5), Cm(3.0)]):
    ...
    _cell_text(t_sig.rows[0].cells[0], "{{ company.director_full_name_ru }}",
               bold=True, align=WD_ALIGN_PARAGRAPH.LEFT, vertical="top")
