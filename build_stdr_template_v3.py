r"""
Pack 50.9-C-r3 — Генератор шаблона stdr_template.docx по эталону СФР.

Изменения r3:
  - Добавлены 2 строки "Подано заявление о..." с датами справа.
  - Заменён метод границ таблицы: tblBorders на уровне таблицы (как в эталоне),
    вместо tcBorders на каждой ячейке. Корректно рендерит merged cells.
...
def _build_applicant_section(doc):
    ...
    # Pack 50.9-r3: 2 строки "Подано заявление о..." с датами справа
    # Layout: label слева | дата справа
    #         "Дата подачи" под датой (мелким курсивом)
    t_stmt = doc.add_table(rows=4, cols=2)
    t_stmt.autofit = False
    _set_table_fixed_layout(t_stmt)
    _apply_widths(t_stmt, [USABLE_WIDTH_CM - 4.0, 4.0])

    _cell_text(
        t_stmt.rows[0].cells[0],
        "Подано заявление о продолжении ведения трудовой книжки",
        size=FONT_SIZE_LABEL_PT,
    )
    _cell_text(
        t_stmt.rows[0].cells[1],
        "{{ stdr.statement_continue_date }}",
        size=FONT_SIZE_LABEL_PT, bold=True,
        align=WD_ALIGN_PARAGRAPH.RIGHT,
    )
    _cell_text(t_stmt.rows[1].cells[0], "", size=FONT_SIZE_LABEL_PT)
    _cell_text(
        t_stmt.rows[1].cells[1], "Дата подачи",
        size=FONT_SIZE_HEADER_PT, italic=True,
        align=WD_ALIGN_PARAGRAPH.RIGHT,
    )
    _cell_text(
        t_stmt.rows[2].cells[0],
        "Подано заявление о предоставлении сведений о трудовой деятельности",
        size=FONT_SIZE_LABEL_PT,
    )
    _cell_text(
        t_stmt.rows[2].cells[1],
        "{{ stdr.statement_provide_date }}",
        size=FONT_SIZE_LABEL_PT, bold=True,
        align=WD_ALIGN_PARAGRAPH.RIGHT,
    )
    _cell_text(t_stmt.rows[3].cells[0], "", size=FONT_SIZE_LABEL_PT)
    _cell_text(
        t_stmt.rows[3].cells[1], "Дата подачи",
        size=FONT_SIZE_HEADER_PT, italic=True,
        align=WD_ALIGN_PARAGRAPH.RIGHT,
    )

    # Без границ
    for row in t_stmt.rows:
        for cell in row.cells:
            _set_cell_borders(cell, top=False, bottom=False, left=False, right=False)
...
def main() -> int:
    print(f"Building STDR template v3 → {OUTPUT_PATH}")
    ...
    if OUTPUT_PATH.exists():
        backup = OUTPUT_PATH.with_suffix(
            f".docx.bak_pre_pack50_9_C_r3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
