r"""
Pack 50.12-C-r6 — Генератор soo_template.docx (Свидетельство об отъезде).
...
def _section_header(doc, num, title):
    ...
    # Pack 50.12 r6 (п.5): пространство между заголовком и блоком
    sep = doc.add_paragraph()
    sep.paragraph_format.space_before = Pt(0)
    sep.paragraph_format.space_after = Pt(0)
    sep.paragraph_format.line_spacing = 0.5
    return t
...
def build_template() -> Document:
    ...
    for row in t2.rows:
        ...
        for c in row.cells:
            _set_cell_margins(c, top=20, bottom=20, left=60, right=60)
    ...
    # п.3: "Трудовой договор" — отдельный параграф с отступом слева (как у Регины)
    p3 = c3.add_paragraph()
