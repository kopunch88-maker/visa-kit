...
// Pack 41.0-D — мультипаспорт: passports[] + passport_id_for_ru_docs
  ...
  // Pack 51 — append-режим: дополнить выписку за указанный суб-период
  ...
  // Pack 51 — дополнить выписку за период [appendFrom, appendTo] без перезаписи
  async function handleAppendBankStatement() {
    if (!application) return;
    if (!appendFrom || !appendTo) {
      setError("Укажите обе даты периода (С и По)");
      return;
    }
    if (appendFrom > appendTo) {
    ...
    setError(null);
    try {
      await appendBankTransactions(application.id, appendFrom, appendTo);
      setAppendEnabled(false);
      setAppendFrom("");
      setAppendTo("");
      if (onApplicationSaved) onApplicationSaved();
    } catch (e) {
      setError(`Не удалось дополнить выписку: ${(e as Error).message}`);
    } finally {
      setBankRegenerating(false);
    }
  }
