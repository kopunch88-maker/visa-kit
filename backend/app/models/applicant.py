...
class Applicant(TimestampMixin, table=True):
    ...
    # Pack 73.11 — желаемый исходящий остаток на счету (для шенген-выписки).
    # Если задано: генератор подгоняет opening_balance или карточные расходы
    # так чтобы closing_balance был ~равен этому значению. NULL = автогенерация
    # как обычно (рандомный остаток).
    target_closing_balance_rub: Optional[Decimal] = Field(
...
class ApplicantCreate(SQLModel):
    ...
    target_closing_balance_rub: Optional[Decimal] = None
