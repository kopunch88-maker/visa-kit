...
Pack 20.0 (04.05.2026): убрано relationship `positions` — Position больше
не привязан к Company. Связь Company↔Position идёт через Application
...
class Company(TimestampMixin, table=True):
    ...
    # Pack 50.8-A — Справка 2-НДФЛ: ОКТМО + телефон компании
    oktmo: Optional[str] = Field(
        default=None,
        max_length=11,
        description="ОКТМО (8 или 11 цифр) — §1 справки 2-НДФЛ",
    )
    ...
    # Pack 20.0: relationship `positions` УБРАН. Position больше не имеет
    # company_id. Если нужно получить позиции, использовавшиеся компанией —
    # JOIN через Application: SELECT DISTINCT p.* FROM position p
    # JOIN application a ON a.position_id = p.id WHERE a.company_id = X.
