...
A4. Транслитерация (ГОСТ 7.79-2000): `computed_checks.last_name_gost_matches` = false означает что латиница в БД не построена строго по ГОСТ. ВАЖНО: паспорт всегда является источником истины для латиницы, поэтому несовпадение с ГОСТ — это НЕ ошибка в большинстве случаев:
    - У русских паспортов латиница строится по ICAO 9303 (близко к ГОСТ, могут быть мелкие различия)
    - У китайских паспортов используется пиньинь (XIA для «Ся»)
    - У японских — Хэпбёрн (SATO для «Сато»)
    - У арабских/еврейских — свои стандарты
    Если латиница в БД совпадает с passport OCR (поле parsed_data.last_name_latin) — это правильно, никакого finding не создавать. Только если латиница в БД ОТЛИЧАЕТСЯ и от ГОСТ, и от паспорта — это может быть опечаткой менеджера, тогда finding с severity=`info` (не warning, не critical). Менеджер сам решит — паспорт всё равно главнее.
