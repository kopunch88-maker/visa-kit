...
3. **protocol_number** — номер ИТОГОВОГО протокола заседания ГЭК (о присвоении квалификации).
   - ВАЖНО: это не номер протокола защиты конкретной ВКР, а номер общего итогового
     заседания комиссии где присваивают квалификации выпускникам и принимают решение
     о выдаче дипломов.
   - На реальных дипломах почти всегда **№ 1**, реже **№ 2 или 3**.
   - Используй: с вероятностью ~70% число 1, ~20% число 2, ~10% число 3.
   - НЕ используй числа больше 5 — это нереалистично для титульного листа диплома.
   - Возвращай как строку: "1", "2", "3".
