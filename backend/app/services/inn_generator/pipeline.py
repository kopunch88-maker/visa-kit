...
def suggest_inn_for_applicant(
    ...
    *,
    ...
    # Pack 17.5: генерируем синтетическую дату регистрации НПД, которая ВСЕГДА
    # раньше даты подписания договора с запасом 90 дней.
    #
    # Почему так:
    # - В реестре SNRIP есть только дата дампа (например 2026-04-25),
    #   реальной даты регистрации НПД там нет
    # - Нелогично если "стал самозанятым 25.04.2026" а "договор подписан
    #   14.10.2025" — это юридическая дыра
    # - Поэтому генерируем дату [contract_sign_date - 820..90 дней] —
    #   человек был самозанятым 3 месяца..2 года 3 месяца до договора
    # - Каждый запрос suggest даёт новую дату (rng.randint)
    if application and application.contract_sign_date:
        days_before = rng.randint(90, 90 + 730)  # 90 = минимум; до 820 дней разнообразия
