...
def build_context(application: Application, session: Session) -> dict[str, Any]:
    ...
    return {
        "applicant": {
            ...
            "full_name_dative": applicant.full_name_dative or _full_name_native(applicant),
...
def build_stdr_context(
    ...
    for rec in parsed_records:
        ...
        # Pack 50.37 (правка юриста к 50.32): ПОЛНОЕ дублирование.
        #   ВЕРХНЯЯ — приём (реальная дата 2019) + увольнение (первая строка
        #   ОБЯЗАТЕЛЬНО приём, даже если приём в 2019 — так в реальных ЭТК).
        #   НИЖНЯЯ — период start..31.12.2019.
        table2_periods.append(_t2_row(real_start, _CUTOFF_LAST))
        _t1_priem()  # Pack 50.37: приём в верхнюю с реальной датой 2019
        if real_end is not None and real_end >= STDR_CUTOFF:
            _t1_uvol(real_end)
