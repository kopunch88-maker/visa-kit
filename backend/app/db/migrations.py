...
# Pack 50.41 — applicant.full_name_genitive + full_name_instrumental

# (родительный/творительный падеж ФИО для письма работодателя при найме)

# ============================================================================

def apply_pack50_41_name_cases_migration() -> None:

    """Pack 50.41 — добавляет applicant.full_name_genitive (родительный) и

    full_name_instrumental (творительный) для письма работодателя (найм):

    'Размер вознаграждения <кого>' и 'Трудовой договор с <кем>'.
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text(
            ...
            "ADD COLUMN IF NOT EXISTS full_name_genitive VARCHAR(128)"
        ...
        conn.execute(text(
            ...
            "ADD COLUMN IF NOT EXISTS full_name_instrumental VARCHAR(128)"

        ))



    print("[migration] Pack 50.41: applicant.full_name_genitive + full_name_instrumental ready")


# ============================================================================
# Pack 50.42 — applicant.full_name_dative (дательный падеж ФИО для письма
# работодателя: 'разрешает выполнение работ <кому>')
# ============================================================================
def apply_pack50_42_dative_migration() -> None:
    """Pack 50.42 — добавляет applicant.full_name_dative. Идемпотентна."""
    from sqlalchemy import text
    from app.db.session import engine
    ...
    with engine.begin() as conn:
        conn.execute(text(
            "ALTER TABLE applicant "
            "ADD COLUMN IF NOT EXISTS full_name_dative VARCHAR(128)"
        ))

    print("[migration] Pack 50.42: applicant.full_name_dative ready")
...
def apply_pack_cv_auto_migration():
    """Pack CV-AUTO — добавляет 3 поля в Position для автогенерации блоков CV.

    Поля:
      - cv_skills_summary_ru   TEXT          (1-3 предложения о навыках)
      - cv_hobbies_pool_ru     JSONB         (пул из 6 хобби)
      - cv_certificates_pool   JSONB         (пул из 6 сертификатов с year_offset)

    Идемпотентно. Существующие позиции после миграции получают NULL —
    их нужно обратно заполнить через Patcher C (backfill через LLM).
    """
    from sqlalchemy import text
    from app.db.session import engine
    ...
    with engine.begin() as conn:
        conn.execute(text(
            "ALTER TABLE position ADD COLUMN IF NOT EXISTS cv_skills_summary_ru TEXT"
        ))
        conn.execute(text(
            "ALTER TABLE position ADD COLUMN IF NOT EXISTS cv_hobbies_pool_ru JSONB"
        ))
        conn.execute(text(
            "ALTER TABLE position ADD COLUMN IF NOT EXISTS cv_certificates_pool JSONB"
        ))
    print("[migration] Pack CV-AUTO: position.cv_skills_summary_ru/cv_hobbies_pool_ru/cv_certificates_pool ready")
