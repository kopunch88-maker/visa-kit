r"""
Pack 50.12-C-r3 — Генератор soo_template.docx (Свидетельство об отъезде).

Версия r3 — 1:1 с эталоном Регины:
...
def _section_header(doc, num, title):
    ...
    _set_cell_width(t.rows[0].cells[0], 0.7)
    _set_cell_width(t.rows[0].cells[1], CONTENT_W_CM - 0.7)

    # Квадратик с рамкой
    c0 = t.rows[0].cells[0]
    _cell(c0, [(num, False)], align=WD_ALIGN_PARAGRAPH.CENTER, valign="center")
    ...
    # Заголовок (без рамки)
    c1 = t.rows[0].cells[1]
    _cell(c1, [(title, True)], align=WD_ALIGN_PARAGRAPH.LEFT, valign="center")
    ...
    t.rows[0].height = Cm(0.55)
    return t


def _bordered_table(doc, rows, cols):
    """Таблица с границами (Table Grid)."""
...
def build_template() -> Document:
    ...
    _add_p(doc, [("Дата ", True), ("{{ soo.date_long }}", True), ("г.", True)],
           space_after=1)
    _add_p(doc, [("№ ", True), ("{{ soo.number }}", True)], space_after=3)
    ...
    for row in t1.rows:
        for ci, c in enumerate(row.cells):
            ...
            _set_cell_margins(c, top=15, bottom=15, left=60, right=60)

    # R0 метки
    _cell(t1.rows[0].cells[0], [("1.1. Фамилия", True)], valign="top")
    ...
    w_num, w_body = 1.0, CONTENT_W_CM - 1.0
    for row in t2.rows:
        _set_cell_width(row.cells[0], w_num)
        _set_cell_width(row.cells[1], w_body)
        for c in row.cells:
            _set_cell_margins(c, top=40, bottom=40, left=60, right=60)  # просторнее
    ...
    ], align=WD_ALIGN_PARAGRAPH.JUSTIFY, space_after=3, line=1.0)
    p2 = c3.add_paragraph()
    _p_runs(p2, [
