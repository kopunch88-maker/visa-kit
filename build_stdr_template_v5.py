r"""
Pack 50.9-C-r5 — Генератор шаблона stdr_template.docx по эталону СФР.

Изменения r5:
  - ФИО, ДР, СНИЛС значения с underline (подчёркивающая линия до конца строки),
    как в эталоне Орлова.
  - Блок ЭЦП как настоящий Drawing TextBox с синей рамкой и синим текстом
    (color 1C6EB5), позиционированный справа внизу, как в эталоне.
...
def _build_applicant_section(doc):
    ...
    # Pack 50.9-r5: ФИО как параграфы — label + tab + underlined value (как эталон).
    # Метка слева ~3.5 см от начала, значение с underline тянется до правой границы.
    from docx.enum.text import WD_TAB_ALIGNMENT as _WD_TAB_ALIGN

    LABEL_TAB_CM = 3.5  # Position of value start (after label)
    ...
    for label, value in rows_data:
        ...
        _set_run(r_label, size=FONT_SIZE_LABEL_PT)
        # Таб к значению
        r_tab = p.add_run("\t")
        _set_run(r_tab, size=FONT_SIZE_LABEL_PT)
        # Значение с underline
    ...
    def _add_statement_paragraph(label_text, placeholder):
        ...
        r_tab = p.add_run("\t")
        _set_run(r_tab, size=FONT_SIZE_LABEL_PT)
        # Плейсхолдер даты — с underline
        r_date = p.add_run(placeholder)
        _set_run(r_date, size=FONT_SIZE_LABEL_PT, bold=True)
        # underline
    ...
    def _add_date_label_paragraph():
        """Параграф 'Дата подачи' справа, мелким курсивом."""
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.paragraph_format.space_before = Pt(0)
        p.paragraph_format.space_after = Pt(2)
        r = p.add_run("Дата подачи")
        _set_run(r, size=6, italic=True)
...
def _add_ep_signature_drawing(doc):
    """Pack 50.9-r5: Drawing TextBox с электронной подписью (синяя рамка + текст)."""
    ...
    # Размеры в EMU (1 cm = 360000 EMU, 1 inch = 914400 EMU)
    # Эталон: cx=2819400 (7.83 см), cy=906780 (2.52 см)
    SHAPE_W_EMU = 2819400
    SHAPE_H_EMU = 906780
    # Позиция: справа, отступ от левого края страницы.
    # Эталон: posOffset=7403058 для альбомного A4 (29.7 см = 10692000 EMU).
    # У нас page_width = 29.7 cm = 10692000 EMU.
    # Чтобы было справа: position_h = page_width - shape_w - small margin
    POS_H_EMU = 10692000 - SHAPE_W_EMU - 360000  # ~30 cm - 7.83 - 1 см
    POS_V_EMU = 100000  # отступ сверху относительно параграфа

    # XML-конструктор: drawing с TextBox и синим обрамлением
    # Многострочный текст в txbxContent через несколько w:p
    color_hex = "1C6EB5"
...
def main() -> int:
    print(f"Building STDR template v5 → {OUTPUT_PATH}")
    ...
    if OUTPUT_PATH.exists():
        backup = OUTPUT_PATH.with_suffix(
            f".docx.bak_pre_pack50_9_C_r5_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
