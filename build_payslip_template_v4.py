r"""
Pack 50.10-C-r4 — Генератор payslip_template.docx, точно по эталону.

Изменения r4 относительно r3:
  - Перераспределены ширины колонок: "Часы" и обе "Сумма" расширены
    за счёт "Вид"-колонок, чтобы текст не переносился
  - Добавлен noWrap в ячейки таблицы (запрет переноса)
...
# Ширины колонок таблицы (Pack 50.10-C-r4: перераспределены)
# Вид/Вид2 сужены, Часы и обе Сумма расширены чтобы текст не переносился.
# Колонки: Вид Период Дни Часы Оплачено Сумма Вид2 Период2 Сумма2
RAW_WIDTHS = [3.0, 1.7, 0.9, 1.3, 1.7, 2.4, 3.2, 1.7, 2.4]
...
def _set_paragraph_bottom_border(paragraph, *, style="single", sz="6", color="333333"):
    """Pack 50.10-C-r3: нижняя граница абзаца (линия под текстом).

    style: 'single' (сплошная) или 'dashed' (пунктир).
    """
    pPr = paragraph._p.get_or_add_pPr()
    pBdr = pPr.find(qn("w:pBdr"))
    if pBdr is None:
        pBdr = OxmlElement("w:pBdr")
        # pBdr должен идти в определённом месте pPr — после tabs/spacing, до rPr
        pPr.append(pBdr)
...
def _cell_text(cell, text, *, size=FONT_SIZE_BASE, bold=False, italic=False,
               align=WD_ALIGN_PARAGRAPH.LEFT, vertical="center", nowrap=False):
    cell.text = ""
    ...
    # Pack 50.10-C-r4: запрет переноса текста в ячейке
    if nowrap:
...
def build_template() -> Document:
    ...
    _add_paragraph(
        ...
        size=FONT_SIZE_BASE,
        space_before_pt=2, space_after_pt=0,
    )
    ...
    p_k_vyplate.paragraph_format.space_after = Pt(0)
    ...
    _cell_text(t.rows[1].cells[3], "Часы",
               size=FONT_SIZE_BASE, align=WD_ALIGN_PARAGRAPH.CENTER, vertical="center", nowrap=True)
    ...
    _cell_text(t.rows[3].cells[5], "{{ payslip.salary_amount_fmt }}",
               size=FONT_SIZE_BASE, align=WD_ALIGN_PARAGRAPH.RIGHT, vertical="center", nowrap=True)
    ...
    # Pack 50.10-C-r3: + сплошная нижняя линия под этим параграфом
    ...
    _set_run(r, size=FONT_SIZE_BASE)
    # Сплошная линия под "Долг предприятия"
    _set_paragraph_bottom_border(p_dolg, style="single", sz="6", color="333333")

    # Параграф с нарастающим итогом — отдельная строка
    # Pack 50.10-C-r3: + пунктирная нижняя линия
    ...
    _set_run(r, size=FONT_SIZE_BASE)
    # Пунктирная линия под "Общий облагаемый доход"
    _set_paragraph_bottom_border(p_total, style="dashed", sz="6", color="333333")
