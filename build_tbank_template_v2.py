...
def _add_jinja_marker_paragraph(doc, marker_text):
    """
    Pack 48.2: добавляет параграф-маркер для docxtpl-условий.

    docxtpl распознаёт {%p ... %} / {%p endif %} как параграф-уровневые
    маркеры — параграф ЦЕЛИКОМ удаляется из вывода после обработки.
    Это позволяет обернуть несколько последующих параграфов (или таблицу)
    в условный блок.

    Маркер вставляется как обычный run без форматирования.
    """
    p = doc.add_paragraph()
    _set_paragraph_spacing(p, before=0, after=0, line=200, line_rule="auto")
    p.add_run(marker_text)
    return p
...
def build_template() -> None:
    ...
    # Адрес места жительства / Адрес (адаптивно по гражданству).
    # Pack 48.2: для RUS — два разных адреса (места жительства + регистрации),
    # для не-RUS — один общий "Адрес" (т.к. в нашей БД одно поле home_address).
    _add_jinja_marker_paragraph(doc, '{%p if applicant.nationality == "RUS" %}')
    ...
    _add_run_simple(p_adj, "{{ applicant.home_address }}",
                    size_pt=9, color=COLOR_BLACK)
    # нижняя граница
    ...
    pBdr.append(bot)
    pPr.append(pBdr)
    _add_jinja_marker_paragraph(doc, '{%p else %}')
    # Альтернатива для иностранцев: лейбл "Адрес:" (без "места жительства")
    p_adr_alt = doc.add_paragraph()
    _set_paragraph_spacing(p_adr_alt, before=120, after=120, line=280, line_rule="auto")
    _add_run_simple(p_adr_alt, "Адрес: ",
                    size_pt=9, bold=True, color=COLOR_BLACK)
    _add_run_simple(p_adr_alt, "{{ applicant.home_address }}",
                    size_pt=9, color=COLOR_BLACK)
    pPr = p_adr_alt._element.get_or_add_pPr()
    pBdr = OxmlElement("w:pBdr")
    bot = OxmlElement("w:bottom")
    bot.set(qn("w:val"), "single")
    bot.set(qn("w:sz"), "4")
    ...
    pBdr.append(bot)
    pPr.append(pBdr)
    _add_jinja_marker_paragraph(doc, '{%p endif %}')

    # Паспорт: 4 поля в одной таблице (Серия | Номер | Дата выдачи | Код подразделения)
    # Pack 48.2: показывается только для RUS-граждан. Для иностранцев —
    # компактная альтернатива ниже.
    _add_jinja_marker_paragraph(doc, '{%p if applicant.nationality == "RUS" %}')
    ...
    pass_labels_values = [
        ...
        ("Дата выдачи: ", "{{ passport_issue_date_str }}"),
    ...
    _add_jinja_marker_paragraph(doc, '{%p else %}')
    # Альтернатива для иностранцев: компактная одна строка
    # "Номер паспорта: <number>    Дата выдачи: <date>"
    p_pass_alt = doc.add_paragraph()
    _set_paragraph_spacing(p_pass_alt, before=120, after=120, line=280, line_rule="auto")
    _add_run_simple(p_pass_alt, "Номер паспорта: ",
                    size_pt=9, bold=True, color=COLOR_BLACK)
    _add_run_simple(p_pass_alt, "{{ applicant.passport_number }}",
                    size_pt=9, color=COLOR_BLACK)
    _add_run_simple(p_pass_alt, "        Дата выдачи: ",
                    size_pt=9, bold=True, color=COLOR_BLACK)
    _add_run_simple(p_pass_alt, "{{ passport_issue_date_str }}",
                    size_pt=9, color=COLOR_BLACK)
    # нижняя граница
    pPr = p_pass_alt._element.get_or_add_pPr()
    pBdr = OxmlElement("w:pBdr")
    bot = OxmlElement("w:bottom")
    bot.set(qn("w:val"), "single")
    bot.set(qn("w:sz"), "4")
    ...
    # "Паспорт выдан:" — общий блок (passport_issuer есть и у RUS и у иностранцев)
    ...
    _add_run_simple(p_pi, "Паспорт выдан: ",
                    size_pt=9, bold=True, color=COLOR_BLACK)
    _add_run_simple(p_pi, "{{ passport_issuer }}",
                    size_pt=9, color=COLOR_BLACK)
    ...
    # Адрес регистрации — только для RUS (для иностранцев у нас один home_address,
    # уже показан выше как просто "Адрес").
    _add_jinja_marker_paragraph(doc, '{%p if applicant.nationality == "RUS" %}')
    ...
    _add_run_simple(p_areg, "Адрес регистрации: ",
                    size_pt=9, bold=True, color=COLOR_BLACK)
    _add_run_simple(p_areg, "{{ applicant.home_address }}",
                    size_pt=9, color=COLOR_BLACK)
