...
> **Дата последнего обновления:** 22.05.2026 — Pack 41-42 (multi-passport на applicant + UX-улучшения админки).
...
- **Pack 25.0-25.7** — починка bank_statement, abbreviate_address во ВСЕХ документах, нумерация акт/счёт по месяцу периода, DN-наниматель первой записью в CV

## Сессия 06.05.2026 — банковская выписка v2 + UI + DOCX-импорт компаний

- **Pack 25.8-25.12** — переработка `bank_statement_generator.py`, миграция `bank_statement_date`, UI секция в `ApplicantDrawer`, DIPLOMA_MAIN всегда замещает `applicant.education`
- **Pack 26.0** — DOCX-импорт реквизитов компании через LLM
- **Pack 27.0** — Корзина с автоудалением через 7 дней

## Сессия 07.05.2026 — Pack 28: пул чистых самозанятых из rmsp-pp

Расследование вскрыло: SNRIP-дамп ФНС — только ИП, не самозанятые. 546k ИНН потенциально засвечивают клиентов через гугл.

- **Pack 28 Часть 1** — `npd_candidate` таблица, `EgrulChecker`, `npd_pool.refill_pool_for_region`, CLI `refill_npd_pool`
- **Часть 2 отложена**: переключение `pipeline.suggest_inn_for_applicant`, cron, admin UI
- **Открытие:** ФНС урезали NPD API — `registrationDate` больше не возвращается (Инцидент 19)
...
Главные паки: **37.0-A/B/C/D** (БД, прompts, auditor, UI), **37.1** (DOCX export), **37.2/37.6/37.7/37.8** (sync work_history с БД), **37.3** (passport_expiry_date), **37.5** (ГОСТ-чек → info severity).
...
## Сессия 24.05.2026 — Pack 43.0 — LLM-перевод полей Position (RU → ES)

**Проблема:** при добавлении новой должности менеджер заполнял только русские поля `tech_opinion_*`. Испанская часть документа `17_Техническое_заключение.docx` рендерилась с пустыми блоками (см. файл `17_Техническое_заключение__7_.docx` от Костиного клиента Цацай — в DICTAMEN TÉCNICO §1, §2, §3, §4 зияли дыры). Эталон правильного рендера — Алиев (id=13), у которого все 6 ES-полей заполнены руками через БД ещё в Pack 40.0-D seed.

**Решение:** кнопка «✨ Сгенерировать испанский» в редакторе должности (`TechOpinionSection`). Один LLM-вызов через Sonnet 4.6 (OpenRouter, через существующий `get_llm_client()`) переводит сразу 5 полей. В БД не пишет — менеджер видит результат в форме, может править, потом сохраняет через обычный PATCH.

**Pack 43.0-A — Backend:**
- Новый файл `backend/app/services/position_translator.py` — `async translate_position_to_spanish(position) -> dict`
- Системный промпт с правилами сохранения структуры массивов (`tools_es`, `steps_es`), не-перевод названий софта (AutoCAD, Power BI, SAP), формальный испанский визовый стиль
- Pydantic-схема `_TranslatedFields` валидирует выход (min_length=1 для каждого массива)
- Новый endpoint `POST /admin/positions/{position_id}/translate-spanish` в `backend/app/api/positions.py`
- НЕ сохраняет в БД — возвращает dict с ES-полями для UI

**Pack 43.0-B — Frontend:**
- `frontend/lib/api.ts` — функция `translatePositionToSpanish(positionId)`
- `frontend/components/admin/settings/TechOpinionSection.tsx` — кнопка ✨ справа от Language tabs. State `translating` + `translateError`. При клике: `window.confirm()` если есть непустые ES-поля → fetch → `onChange(...)` с новыми ES-полями → авто-переключение на вкладку Español
- Кнопка **disabled** при `positionId == null` (новая несохранённая должность) с tooltip «Сначала сохраните должность»
- `frontend/components/admin/settings/PositionDrawer.tsx` — передан `positionId={positionId}` в `<TechOpinionSection>`

**28 старых должностей не тронуты** — у них tech_opinion_es-поля уже заполнены руками через БД (Pack 40.0-D seed + Костя добивал точечно).

**Стоимость:** ~$0.02-0.05 за один LLM-вызов (~3-4К токенов вход, ~2-3К токенов выход). Точки вызова — только когда менеджер жмёт кнопку.
...
| **41.0-D** | UI Drawer: новый компонент `PassportsSection.tsx` (~430 строк) с inline-редактированием, бейджами, dropdown «🇷🇺 Паспорт для договора», кнопкой «Использовать внутренний РФ», кнопкой «Резолв issuer_ru». Костя руками сделал 5 правок в `ApplicantDrawer.tsx`: импорт `PassportsSection`, замена 5 useState, удаление `handleResolvePassportIssuerRu`, замена 5 полей в payload, замена `<Section title="Паспорт">` на `<PassportsSection>` + сохранение `birth_date/birth_place_latin/birth_country` в новой секции «Место и дата рождения» | ✅ В проде |
| **41.0-E + fix1/fix2/fix3_v2** | Русские DOCX-рендеры. Helper `get_passport_dict_for_ru_docs(applicant)` с 3-уровневым приоритетом. Изначальный patcher имел баги — многострочные якоря с пустыми строками молча no-op'ились. fix2 применил недостающие правки через codepage-safe single-line якоря с len-delta проверкой. fix3_v2 (через AST `end_lineno`) добавил недостающий импорт после многострочного `from app.services.bank_statement_generator import (...)`. **Инцидент 42, Правило 64** | ✅ В проде |
| **41.0-G + fix1/fix2** | БИЗНЕС-ЛОГИКА ИЗМЕНЕНА: `passport_id_for_ru_docs` применяется ТОЛЬКО для договора. `context.py` откатился на primary (как было до 41.0-E), `context_npd_certificate.py` откатился. В `docx_renderer.py render_contract()` добавлен override блок который перезаписывает `context["applicant"]` паспортными полями из `_ru_passport`. fix1 — label «🇷🇺 Паспорт для договора» вместо «Паспорт для русских документов». fix2 СРОЧНЫЙ — конвертация `_ru_passport["issue_date"]` из ISO-string в date через `date.fromisoformat()` (был `AttributeError 'str' has no attribute 'strftime'` в проде). **Инцидент 43, Правило 65** | ✅ В проде |
...
| Pack | Что | Результат |
|---|---|---|
| **42.0** | Убран авто-статус «Документы готовы» (`DRAFTS_GENERATED`) после `render_package` в `applications.py`. Менеджер сам выставляет статус через dropdown когда проверил. Фронт-вызов `updateStatus(applicationId, "drafts_generated")` в `ApplicationDetail.tsx:282` НЕ ТРОНУТ — это откат при снятии «Подан» | ✅ В проде |
| **42.1** | Кнопка 🗑 «Удалить» документ клиента. Backend: DELETE `/admin/applications/{app_id}/client-documents/{doc_id}` (удаление из БД + R2 storage). API: `adminDeleteClientDocument`. UI: красная кнопка с `window.confirm()` подтверждением в карточке документа | ✅ В проде |
| **42.2** | Паспорта НЕ перезаписываются друг другом при загрузке. В `import_package.py` функция `_upsert_document_for_application` искала existing по `(application_id, doc_type)` и удаляла старый. Теперь обёрнуто в `if doc_type_enum not in _PASSPORT_MULTI_TYPES`. `_PASSPORT_MULTI_TYPES = {PASSPORT_NATIONAL, PASSPORT_FOREIGN, PASSPORT_INTERNAL_MAIN, PASSPORT_INTERNAL_ADDRESS}` — каждый паспорт = отдельная `ApplicantDocument` запись | ✅ В проде |
| **42.3** | Drag-and-drop в секцию «Документы клиента». `ImportPackageDialog` расширен props `initialFiles?: File[]` + `initialApplicationId?: number` + useEffect для quick-mode (target="existing" + handleFilesSelected минуя UploadStep). В `AdminClientDocuments`: вся секция = drop-zone с синей подсветкой при drag-over + overlay «Отпустите файлы для импорта». При drop → открывается ImportPackageDialog с pre-loaded файлами и текущей заявкой. После import — auto-refresh документов | ✅ В проде |

**Главный итог сессии 22.05.2026:** Multi-passport support end-to-end + 4 UX-улучшения в админке (статус, удаление документов, multi-passport upload, drag-and-drop).
...
**Pack 37.3:** `passport_expiry_date` теперь в `OCR_FIELD_MAP` в двух местах.

**Pack 41.0-C:** OCR auto-добавление в `applicant.passports[]`. Распознанные паспортные данные не только записываются в legacy скаляры `applicant.passport_*`, но и upsert-ятся в массив `passports[]` через `upsert_by_number` (нормализация номера, dedup, пересчёт primary).
...
**Стоимость:** ~$0.10 за прогон (в 1.7× дешевле Pack 37.0).
...
**Применяется:** менеджер открыл карточку клиента → перетянул файлы из проводника на секцию «Документы клиента» → сразу запустился import (без выбора файлов и заявки).

---

## 3.16 Tech Opinion подсистема (Pack 40.0-G + Pack 43.0)
...
**Рендерер:** `templates_engine/docx_renderer.py:render_tech_opinion(application, session)`
**Контекст-сборка:** `templates_engine/context.py:1640-1652` (RU-блок) + `1683-1693` (application-level)
...
**Pack 43.0 — кнопка «✨ Сгенерировать испанский» в редакторе должности:**
- Endpoint `POST /admin/positions/{id}/translate-spanish` (`backend/app/api/positions.py`)
- Сервис `backend/app/services/position_translator.py` — один LLM-вызов Sonnet 4.6, JSON-выход, Pydantic-валидация
- НЕ пишет в БД — возвращает dict с ES-полями для UI, менеджер сам сохраняет через PATCH
- Кнопка disabled для новой несохранённой должности (нет id для endpoint'а)
- `window.confirm()` если ES-поля уже непусты
...
director_full_name_latin       — GOST 7.79
...
| `tech_opinion_template.docx` | render_tech_opinion → 17_Техническое_заключение.docx | 40.0-G + **43.0** (LLM-перевод RU→ES) |
...
4. **company id=15 ИНЖГЕОСЕРВИС** содержит мусор. TODO: ручной cleanup.
...
10. **Pack 41.1 (отложено)** — консолидация дублирующейся логики `_auto_apply_ocr_to_applicant` между `client_documents_admin.py` и `import_package.py`.
11. **Pack 41.0-F (отложено)** — AI-аудит (`audit_context_builder.py` + `audit_prompts.py`) должен учитывать `passport_id_for_ru_docs`, иначе ложно срабатывает на договорах с выбранным старым паспортом.
...
- **Pack 39.0-E3** — селект для переключения между несколькими прогонами
- **Pack 39.1** — Badge `📋 Финальная: FAIL/WARN/PASS` в карточке заявки на главной
- **Pack 39.2** — Visual quality check через Vision claude-sonnet-4-5 для печатей/подписей
- **Pack 39.3** — Автоудаление документов при `application.deleted_at`

### Pack 41.0-F — AI-аудит учитывает passport_id_for_ru_docs (отложено)
Сейчас аудит может ложно срабатывать на договорах с выбранным старым паспортом.

### Pack 41.0-C.1 — отдельный паспорт для НПД-справки (если стрельнёт кейс)

### Pack 41.1 — консолидация дублирующейся OCR auto-apply логики
...
✅ **Pack 42.x — UX-полировка админки:**
   - **42.0** — статус «Документы готовы» только вручную через dropdown (не авто после render)
   - **42.1** — кнопка 🗑 Удалить документ клиента (БД + R2) с `window.confirm()`
   - **42.2** — паспорта НЕ перезаписываются при загрузке (каждый паспорт = отдельная запись в `applicant_document`)
   - **42.3** — Drag-and-drop в секцию «Документы клиента»: бросил файлы → открывается ImportPackageDialog с pre-loaded файлами и текущей заявкой (минуя UploadStep и выбор заявки)
✅ **Pack 43.0 — кнопка «✨ Сгенерировать испанский» в редакторе должности:**
   ...
   - 5 полей за раз (description_es, tools_es, steps_es, grounds_es, contract_clause_es)
...
**Все инциденты ниже — это реальные баги в проде или нерабочие сборки.**

## Инцидент 6 (Pack 25.x) — Hardcoded даты в DOCX-шаблоне
30 минут дебага кода, оказалось что в `bank_statement_template.docx` были hardcoded даты. **Правило 28.**

## Инцидент 12 (Pack 27.0 Stage A) — Endpoints забыли зарегистрировать
Apply-скрипт упал silent. Кнопка → 405. **Правило 38.**

## Инцидент 19 (07.05.2026) — ФНС урезали NPD API
`registrationDate` больше не возвращается. Часть 2 Pack 28 отложена. Урок: внешние API могут урезаться без уведомления.

## Инцидент 20 (09.05.2026) — Pack 19.1a/20.3 endpoint забыли 5 дней
`regen-work-history` не был зарегистрирован, но в PROJECT_STATE числился «работает». **Правило 38.**

## Инцидент 21 (10.05.2026) — Position raw SQL INSERT на NOT NULL без DB DEFAULT
SQLModel `default_factory` работает только через ORM. **Правило 43.**
...
## Инцидент 25 (10.05.2026) — «Зрительный обман» в договоре оказался hard line break
Pack 16.7 чинил дефолтный шаблон, per-company скопированы со старой структурой. Pack 33.6 — merge во всех 11 per-company.

## Инцидент 30 (11.05.2026) — Translation worker зомби в IN_PROGRESS
Решение Pack 35.8 — кнопка «Отменить». Backlog: auto-timeout.

## Инцидент 31 (11.05.2026) — Pack 35.4 каскадный NameError
`_build_bank_context` правился, забыли `_generate_fresh_bank_context` без `applicant`. **Правило 54.**
...
`print + pass` вместо `sys.exit(1)`. **Правило 56.**
...
Многострочные якоря с пустыми строками между блоками. Якоря не сматчились, но `str.replace` молча вернул unchanged. Patcher напечатал «applied» для 5 блоков, по факту применилось 2. Серия fix1/fix2/fix3_v2: fix2 применил недостающие правки через codepage-safe single-line якоря с len-delta проверкой, fix3_v2 (через AST `end_lineno`) добавил недостающий импорт после многострочного `from app.services.bank_statement_generator import (...)`. Падало в проде с `NameError: name 'get_passport_dict_for_ru_docs' is not defined`. **Правило 64.**

## Инцидент 43 (22.05.2026) — Pack 41.0-G в проде: `AttributeError: 'str' object has no attribute 'strftime'`
`passports[].issue_date` хранится в JSONB как ISO-string `"2026-04-10"`. Pack 41.0-G override блок в `render_contract` передавал `_ru_passport["issue_date"]` (строку) в `fmt_date_ru()` который вызывает `strftime`. Падало на каждом render договора Каримова (паспорт C05803188). Hotfix fix2 — конвертация через `date.fromisoformat()`. **Правило 65.**
...
**Версия документа:** 4.2 (22.05.2026 — Pack 40.x autogen + Pack 41.0 multi-passport + Pack 42.x UX-полировка. Добавлены: §3.14 multi-passport архитектура, §3.15 drag-and-drop, миграция 41.0-A, Правила 64-65, Инциденты 42-43).

**Базируется на:** 4.1 (20.05.2026 — Pack 39.0 Final Submission Audit) ← 4.0 (18.05.2026 — Pack 37.x AI Document Audit) ← 3.7 (16.05.2026 — Pack 36.0/36.1) ← 3.6 (Pack 35.x) ← 3.5 (Pack 33.x).
