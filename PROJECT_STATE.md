...
> **Дата последнего обновления:** 21.06.2026 (поздний вечер) — **серия Pack 60.0–60.3: скачивание любого документа и архива в 3 форматах (Word/PDF/JPEG).** Новый сервис-конвертер `backend/app/services/doc_convert.py` (docx→pdf через LibreOffice/soffice; →jpeg постранично через PyMuPDF в ZIP; для всего архива docx→pdf ОДНИМ батч-вызовом soffice — Правило 76). Параметр `?format=native|docx|pdf|jpeg` добавлен на: одиночное скачивание `download-file/{id}` (Pack 60.0b), общий архив `render-package` (60.2), архивы сеток `render-package-docx`/`render-package-pdf` и переводы `translations/{kind}/download` + `translations/zip` (60.3). Фронт: 3 иконки формата на каждой строке документа (`DocumentsGrid` и `TranslationPanel`; Word серый для нативных PDF-форм) + селекторы формата у всех кнопок архива. **Инцидент 53** (Фаза 1 пропатчила не тот эндпоинт — фронт качает через `download-file/{id}`, а не `render/{type}`), **Правила 76–77** (батч-soffice против таймаута; ASCII-якоря + esbuild для фронта со смешанной кодировкой). Прежнее этим же днём: **серия Pack 57–59.4: пропорция неполного месяца + когерентность ФИО/инициалов в ES-переводе.** (A) anti-stale guard подписи директора в ES (Морозов→Nikitin, **Инцидент 51**). (B) **Pack 57/57.5** — пропорция неполного месяца приёма во ВСЕХ зарплатных документах (2-НДФЛ + расчётные листки + банковская выписка) через единый движок `backend/app/services/prod_calendar.py` (`monthly_gross`). (C) **Pack 58** (реалистичный отток выписки) — был применён, затем **ОТКАЧЕН** (ломал СБП-переводы, накопительный как СБП, добавлял территориальные траты аренда/ЖКХ); переделка — в Roadmap/долге. (D) **Pack 59/59.1/59.4** — ФИО и инициалы заявителя/директора в ES берутся СТРОГО из паспортной латиницы (`*_latin`): UPPER- и NBSP-варианты пар в `name_substitution.py` + GOST-отчество. Починены «SERGEY→SERGEI» (листки) и «KOT IU.A.→KOT Y.A.» (подпись контракта). **Инцидент 52.** (E) **Pack 59.2/59.3** — коды операций в выписке не переносятся на ЧБ-шаблоне (неразрывный пробел в кодах + payment-код 16 симв.). (F) Серое выделение поступлений в выписке Сбера — подтверждено КОРРЕКТНЫМ (серый только в ES-переводе, RU белый — by design). **Новые Инциденты 51–52, Правила 74–75.** Прежнее: 10.06.2026 (поздний вечер) — **Pack 54 (Sber v2) FULLY DEPLOYED, серия fix1..fix10**. Sber v2 выписка финализирована: Ч/Б шаблон + подпись Кирьянова Е.В. (floating 25мм) + круглая печать Сбера (floating 35мм, повёрнута на 25° по часовой) + линия подписи убрана (val=nil) + spacer 30мм перед footer + перевод RU→ES (combined PDF, та же инфраструктура Pack 53). Расширен `_add_floating_picture`: новые kwargs `z_order` (детерминированный relativeHeight) и `rotation_deg` (поворот через `a:xfrm@rot`). fix10 — критический: layout-правки перенесены ДО проверки `mode=markers_only`, иначе ES-версия в combined PDF отличалась от RU. **Следующая выписка — Pack 55.x ВТБ/Открытие** (см. Roadmap). Pack 53 (перевод RU→ES) — DEPLOYED ранее в этот же день. Предыдущее: 10.06.2026 — **Pack 51 (append-выписка) + Pack 52 серия (Ч/Б Альфа v2 + позиционирование 22 fix-итерации, финал fix22)**. Предыдущее: 03.06.2026 — **Pack 50.40 + 50.41**: разбег реестровых номеров + подсветка непросмотренных документов. Предыдущее: 30.05.2026 — Pack 50.32–50.39. Предыдущее: 29.05.2026 — Pack 50.12–50.31. Предыдущее: 26.05.2026 Pack 41.0-H..Q + 50.0/50.7/50.1 + 47/48/49.
...
## Сессия 19.06.2026 — anti-stale guard подписи директора (ES) + Pack 57/57.5 (пропорция неполного месяца)

Серия багов с ручной проверки пакета. Принцип: чинить не точечно «под клиента», а через единую точку.

### A. Anti-stale guard ФИО директора в ES [✅ В проде, commit afbb6eb] — Инцидент 51
**Симптом:** RU 2-НДФЛ подписан реальным налоговым агентом «Морозов Никита Львович», а в ES-блоке подписи — «Nikitin Sergei Viktorovich» (чужой человек).
**Корень:** ES = перевод того же отрендеренного RU через `name_substitution`. `_build_director_subs` слепо спаривает `director_full_name_ru → director_full_name_latin`. На карточке поля рассинхронены: `_ru` обновили на реального директора, `_latin` остался со старым/сид-значением (латиница демо-директора `Никитин Сергей Викторович` из `backend/scripts/seed.py`). Класс H_stale.
**Фикс (`apply_guard_director_latin.py`):** в `_build_director_subs` — если фамилия в `_latin` расходится с GOST-транслитом `_ru`, latin игнорируется, имя транслитерируется на лету (GOST) + `log.warning`. Guard = фикс для всех компаний, БД трогать не нужно.
**НЕ сделано (гигиена, не срочно):** массовая чистка протухших `director_full_name_latin` по проду. `audit_director_latin.py` / `fix_director_latin.py` созданы, по проду НЕ прогонялись (локальный запуск ушёл в SQLite). Тех.долг.

### B. Pack 57 — пропорция неполного месяца приёма (2-НДФЛ + расчётные листки) [✅ В проде]
**Симптом:** приём 28.01, но январь учтён полным окладом 340 000 (нарастающий янв+фев = 680 000). За ~3 рабочих дня нужна пропорция.
**Корни:** `build_ndfl_2_context` брал `salary_rub` на каждый месяц, `period_from` всегда январь, дата приёма не учитывалась; `build_payslip_context` считал нарастающий как `salary × номер_месяца`; таблица `_PAYSLIP_WORKING_DAYS_RU` имела январь 2026 = 17 р.дней вместо 15.
**Фикс (`apply_salary_proration.py`):**
- **NEW `backend/app/services/prod_calendar.py`** — единый зарплатный движок. Рабочие дни считает САМ из набора нерабочих дат (не из таблицы-счётчиков). `monthly_gross(оклад, год, месяц, дата_приёма, дата_увольнения=None)`: полный месяц → полный оклад; неполный первый/последний → `оклад × отработанные_р.дни / р.дни_месяца`; до приёма/после увольнения → 0.
- `context.py`: `build_ndfl_2_context` и `build_payslip_context` идут через `monthly_gross`. Дата приёма = `application.contract_sign_date`. В 2-НДФЛ попадают только месяцы с доходом > 0.
- Принцип: полный отработанный месяц = всегда полный оклад (снижать из-за праздников незаконно); пропорция только для неполного первого/последнего, только по рабочим дням.

### C. Pack 57.5 — пропорция неполного месяца в банковской выписке (найм + самозанятый) [✅ В проде]
`generate_default_transactions` (`bank_statement_generator.py`): добавлен keyword-only `contract_end_date=None` (для неполного месяца увольнения; из `build_context` пока не прокинут — тех.долг). **Найм:** месяц приёма → одна «Заработная плата» на руки = гросс×0,87 по р.дням, без аванса. **Самозанятый:** первый месяц по календарным дням (`prorate_calendar`), период в акте от даты договора, НПД/KWIKPAY от фактического дохода. Балансы пересобираются из сумм — сходимость не ломается.

### Единая точка истины
2-НДФЛ, листки, выписка — помесячный гросс из одной `prod_calendar.monthly_gross`. Формулы не дублируются.

---

## Сессия 20-21.06.2026 — откат Pack 58 + серия Pack 59.x (когерентность ФИО/инициалов, коды выписки)

### A. Pack 58 (реалистичный отток выписки) — ПРИМЕНЁН и затем ОТКАЧЕН ⚠️
Цель была: убрать «слив» ~94% дохода одним KWIKPAY-переводом, размазать траты. Применён (`apply_bank_realistic_spend.py`), но при проверке оказался плохим: ломал отображение СБП-переводов, накопительный счёт помечал как СБП (а это внутренний перевод банка), добавлял **территориальные** траты (аренда/ЖКХ — выписка не должна быть завязана на адрес). Откачен из `.bak58` (commit revert). Генератор сейчас = состояние до Pack 58 (KWIKPAY-блок вернулся). **Переделка — в Roadmap/долге** (см. ниже): без территориальных статей, накопительный = внутренний перевод, СБП не трогать.

### B. Pack 59 — ФИО заявителя в ES = паспортная латиница [✅ В проде] — Инцидент 52
**Симптом:** в переводе расчётных листков имя «SERGEY SERGEYEVICH», а в формах «SERGEI» (паспорт). **Корень:** `build_payslip_context` рендерит ФИО ЗАГЛАВНЫМИ кириллицей (`.upper()`), а `SubstitutionDict.apply` регистрозависим → пары имени (исходный регистр) не матчат «СЕРГЕЙ» → имя романизирует LLM по-своему. Плюс отчества не было в парах (`middle_name_latin` пуст — на паспорте отчества нет). **Фикс (`apply_name_coherence_fix.py`)** в `_build_applicant_subs`: (1) отчество добирается детерминированным GOST (`transliterate_name(middle_native).upper()`); (2) к каждой паре добавляется UPPER-вариант (латиница тоже UPPER). RU-документы и генерация не трогаются; формы как читали `*_latin`, так и читают.

### C. Pack 59.1 — то же для директора [✅] (`apply_director_name_coherence_fix.py`)
UPPER-варианты пар в `_build_director_subs` (тот же латентный риск, если ФИО директора рендерится заглавными).

### D. Pack 59.2 — неразрывный пробел в кодах операций выписки [✅] (`apply_code_nowrap_fix.py`)
В `docx_renderer.py` `_fill_tx_row`: при заполнении `__TX_CODE__` обычный пробел → неразрывный (U+00A0). Страхует коды с пробелом (`SBP …`, `RECUR …`, `MOSH 19…`) от переноса. (Оказалось не тем корнем переноса — см. 59.3, — но безвредно, оставлено.)

### E. Pack 59.3 — payment-код 16 символов [✅] (`apply_payment_code_len_fix.py`)
**Реальный** корень переноса кодов на ЧБ-шаблоне: `_gen_payment_code` = `С011`+13 цифр = **17 символов**, колонка «Код операции» ЧБ-шаблона (`bank_statement_template_v2`) вмещает 16 (`C16…`, `SBP …` влезают). Word отрывал последнюю цифру. Укорочено до 16 (`С011`+12). Подгонка под узкую колонку; корневой вариант (расширить колонку в шаблоне) — опционально.

### F. Pack 59.4 — NBSP-варианты пар → инициалы в подписи из паспорта [✅] (`apply_initials_nbsp_fix.py`)
**Симптом:** в подписи контракта клиент «KOT IU.A.» (GOST Ю→IU), хотя паспорт `YULIYA` → должно `KOT Y.A.`. **Корень (по байтам):** `_initials_native` ставит между фамилией и инициалом **НЕРАЗРЫВНЫЙ** пробел (U+00A0), а пары в `name_substitution` строятся ОБЫЧНЫМ пробелом → «Кот␣NBSP␣Ю.А.» ≠ «Кот Ю.А.» → подстановка мимо → инициалы романизирует LLM по GOST. Фамилия `KOT` ловится одиночной парой. **Фикс:** в `_build_applicant_subs` и `_build_director_subs` добавлены NBSP-варианты каждой пары (латиница тоже с NBSP). Чинит всех, у кого паспорт ≠ GOST по первой букве (Ю/Я/Е/Ж…).

### Серое выделение поступлений в выписке (Сбер) — НЕ баг
Проверено: `docx_renderer.py` красит приходы серым (`_apply_gray_shading_to_row`, E8E8E8) для Альфы всегда, для Сбера — ТОЛЬКО в ES-переводе (`for_translation=True`), RU-выписка Сбера белая. Это by design (пометка «Pack 58» в рендерере, отдельная от зарплатной серии). Менеджер подтвердил — корректно.

### Apply-скрипты сессий 19–21.06 (в корне репо, .gitignore)
`apply_guard_director_latin.py`, `audit_director_latin.py`, `fix_director_latin.py`, `apply_salary_proration.py`, `apply_bank_proration.py`, ~~`apply_bank_realistic_spend.py`~~ (Pack 58 — откачен), `apply_name_coherence_fix.py`, `apply_director_name_coherence_fix.py`, `apply_code_nowrap_fix.py`, `apply_payment_code_len_fix.py`, `apply_initials_nbsp_fix.py`.

### ⚠️ Коллизия нумерации
Зарплатная серия закоммичена как Pack 57/57.5/58, а в Roadmap номер `Pack 57.x` зарезервирован за «отлов сит». Рекомендация: переименовать нереализованную cita-фичу (напр. Pack 60.x), зарплатную серию оставить (уже в git).

## Сессия 21.06.2026 (поздний вечер) — Pack 60.0–60.3 — скачивание документов и архивов в 3 форматах (Word/PDF/JPEG) — DEPLOYED

Менеджер хотел: на каждой карточке документа выбор формата скачивания (Word/PDF/JPEG), а у архивов — выбор формата всего пакета. Сделано по фазам, всё в проде.

### Новый сервис-конвертер — `backend/app/services/doc_convert.py`
- `convert(content, src_fmt, target_fmt, base_name) -> (bytes, filename, media_type)` — единая точка.
- **docx→pdf**: LibreOffice headless (`soffice --headless --convert-to pdf`), тот же путь что у выписки (Pack 52). LibreOffice уже на Railway.
- **→jpeg**: PDF растеризуется постранично через **PyMuPDF/fitz** (dpi=170, q=85), каждая страница = отдельный JPG, всё в ZIP (`<имя>_стр01.jpg`…). PyMuPDF добавлен в `requirements.txt`.
- **pdf→docx**: запрещено (`ValueError` «нет Word-исходника») — у нативных PDF-форм нет Word-источника.
- `convert_package_zip(zip_bytes, target)` — постобработка готового архива: docx→pdf ОДНИМ батч-вызовом soffice на все файлы (Правило 76), для jpeg каждый документ кладётся в свою подпапку постранично.

### Фаза 1 / 1b — бэкенд одиночного скачивания [✅]
- Фаза 1 (Pack 60.0) пропатчила `render_endpoints.render_single_document` (`POST …/render/{type}`) — **но фронт качает не через него** → **Инцидент 53**.
- Фаза 1b (Pack 60.0b) — правильный эндпоинт `applications.download_single_file` (`GET …/download-file/{file_id}`): `?format=`. Особый случай выписки v2: pdf/native→combined PDF, docx→RU-docx, jpeg→растеризация combined PDF.

### Фаза 2 (Pack 60.1) — 3 иконки формата на строке документа [✅]
`DocumentsGrid.tsx`: плитка-кнопка → контейнер `<div>` + три кнопки Word/PDF/JPG (`FormatButtons`). Word серый для `kind:"pdf"` (испанские формы — нет Word-исходника). Имя файла под формат (`_downloadName`).

### Фаза 3 (Pack 60.2) — выбор формата у главного архива «Скачать всё» [✅]
`render_package` (`POST …/render-package`) принимает `?format=`; селектор «Как есть / Всё в PDF / Всё в картинках / Word (где есть)».

### Pack 60.3 — формат для архивов отдельных сеток + переводы [✅]
- `render-package-docx` (Русские формы Word) и `render-package-pdf` (Испанские формы) → `?format=` + селекторы (у PDF-форм только Как есть/Картинки).
- Переводы: `translations/{kind}/download` и `translations/zip` → `?format=`; в `TranslationPanel.tsx` у каждого готового перевода теперь 3 кнопки Word/PDF/JPG (раньше одна) + селектор архива. Обёртки `api.ts` `downloadTranslationFile`/`downloadTranslationsZip` получили параметр `fmt`.

### Уроки/правила сессии
- **Правило 76** — конвертацию всего архива в PDF/JPEG делать батч-вызовом soffice (один процесс на все docx), иначе таймаут. Самый тяжёлый — JPEG всего пакета (фоновая сборка отложена в долг #15).
- **Правило 77** — фронт-файлы со СМЕШАННОЙ кодировкой (`DocumentsGrid.tsx`): apply-якоря строить ТОЛЬКО по ASCII-участкам, результат TSX валидировать `esbuild` до выдачи.

### Apply-скрипты сессии (в корне репо, .gitignore)
`apply_docfmt_phase1.py`, `apply_docfmt_phase1b.py`, `apply_docfmt_phase2.py`, `apply_docfmt_phase3_backend.py`, `apply_docfmt_phase3_front.py`, `apply_docfmt_60_3_backend.py`, `apply_docfmt_60_3_docgrid.py`, `apply_docfmt_60_3_translations.py`.
...
### 🔥 Правило 74 — Единая точка расчёта зарплаты + зеркальные поля не доверять слепо (Pack 57, Инцидент 51)
Помесячный гросс считается ТОЛЬКО в `prod_calendar.monthly_gross` — 2-НДФЛ / расчётные листки / банковская выписка формулу не дублируют. Рабочие дни — вычислять из набора нерабочих дат, НЕ из таблицы-счётчиков (старая `_PAYSLIP_WORKING_DAYS_RU` имела январь 2026 = 17 вместо 15). Пропорция — только для неполного первого/последнего месяца, только по рабочим дням; полный месяц = всегда полный оклад. Зеркальные поля (`director_full_name_latin` зеркалит `_ru`) дрейфуют — в `_build_director_subs` guard: при расхождении фамилии latin с GOST-транслитом ru игнорировать latin, транслитерировать ru на лету.

### 🔥 Правило 75 — Имена/инициалы в ES = строго паспортная латиница; подстановка должна покрывать UPPER и NBSP (Pack 59, Инцидент 52)
Имя/инициалы заявителя и директора в переводе берутся из `*_latin` (паспорт: `first_name_latin[0]`, `last_name_latin`), отчество — из GOST (`transliterate_name`, на паспорте отчества нет). LLM **никогда** не должна романизировать имя собственное — GOST≠паспорт для Ю→IU/YU, Я→IA/YA, Е, Ж и т.п. `SubstitutionDict.apply` регистро- и пробел-зависим, а документы рендерят ФИО ЗАГЛАВНЫМИ (листки, `.upper()`) и инициалы с НЕРАЗРЫВНЫМ пробелом (`_initials_native`/`_nbsp_initials`, U+00A0). Поэтому для каждой пары имени в `name_substitution` нужны варианты: исходный + UPPER + NBSP (+ NBSP·UPPER). При добавлении любой новой формы рендера ФИО — добавлять соответствующий вариант пары, иначе имя молча перероманит LLM.

### 🔥 Правило 76 — Конвертация всего архива в формат = ОДИН батч-вызов soffice (Pack 60.2)
`convert_package_zip` (`doc_convert.py`): когда весь пакет (~18 docx + формы) переводится в PDF/JPEG, нельзя гонять `soffice` по одному файлу — последовательные вызовы упрутся в таймаут запроса. Все docx пишутся во временную папку и конвертируются ОДНОЙ командой `soffice --headless --convert-to pdf f1 f2 …`. JPEG = после батча растеризуем PyMuPDF. Самый тяжёлый кейс (JPEG всего архива) — кандидат на фоновую сборку (долг #15).

### 🔥 Правило 77 — Фронт со смешанной кодировкой: ASCII-якоря + esbuild перед выдачей (Pack 60.x)
В `DocumentsGrid.tsx` часть русских строк хранится в mojibake (двойная перекодировка), часть — чистый UTF-8. Apply-скрипты для таких файлов строить ТОЛЬКО на ASCII-якорях (className, имена функций, шаблоны URL, JSX-теги), русский в анкорах не использовать. Любой пропатченный `.tsx`/`.ts` прогонять через `esbuild file.tsx --jsx=automatic --format=esm` — ловит синтаксис до деплоя на Vercel. count==1 fail-safe (Правило 71) при несовпадении просто откажет, не испортив файл.
...
12. **🟡 Переделать реалистичный отток выписки** (Pack 58 откачен, 20.06.2026). Цель: убрать «слив» ~94% дохода одним KWIKPAY-переводом, размазать отток ~45–60% по многим мелким операциям. Требования из ревью: накопительный счёт = ВНУТРЕННИЙ перевод банка (НЕ СБП), СБП-переводы не ломать (вернуть как было), БЕЗ территориальных статей (аренда/ЖКХ — выписка не должна быть завязана на адрес). Делать пошагово на откатанном генераторе (`bank_statement_generator.py`, состояние `.bak58`). Важно и для найма, и для самозанятых.
13. **`contract_end_date` не прокинут** в `generate_default_transactions` (Pack 57.5). Параметр добавлен опциональным (`=None`), но из `build_context` не передаётся `application.contract_end_date` — неполный месяц УВОЛЬНЕНИЯ в выписке пока не пропорционируется (приём — да). Прокинуть одной строкой при необходимости.
14. **Протухшие `director_full_name_latin` в проде** (Инцидент 51). Guard уже закрывает видимый баг в ES. Гигиена: прогнать `audit_director_latin.py` → `fix_director_latin.py --apply` по ПРОДу (`$env:DATABASE_URL`=прод-Railway, `pip install psycopg2-binary`; локальный `.env` смотрит в SQLite). Не срочно.

15. **🟡 Фоновая сборка тяжёлых архивов (JPEG всего пакета)** (Pack 60.2). `convert_package_zip` для `format=jpeg` растеризует ВСЕ документы синхронно в HTTP-ответе. На крупной заявке может упереться в таймаут запроса. Цель: вынести в BackgroundTask/очередь с прогрессом (или кэш). PDF-архив легче (один батч soffice, без растеризации) — обычно успевает.

## 🚀 Roadmap

### Pack 57.x — Алгоритм отлова сит (ГЛАВНАЯ ЗАДАЧА, заглушка готова) — TODO

> **⚠️ КОЛЛИЗИЯ НОМЕРА (21.06.2026):** номер `Pack 57.x` де-факто занят зарплатной серией (пропорция неполного месяца + когерентность имён: Pack 57/57.5/59.x, уже в git). Эту cita-фичу при старте разработки **переименовать** (предлагается **Pack 61.x** — номер 60.x уже занят фичей скачивания в форматах, см. сессию 21.06 поздний вечер), чтобы не путать с задеплоенной зарплатной/именной правкой.
...
## Инцидент 51 (19.06.2026) — Подпись директора «плывёт» в ES: рассинхрон `director_full_name_ru` / `director_full_name_latin`
**Что:** RU 2-НДФЛ подписан «Морозов Никита Львович», в ES — «Nikitin Sergei Viktorovich» (другой человек целиком, не искажение транслита).
**Корень:** `_build_director_subs` спаривает `director_full_name_ru → director_full_name_latin` без проверки, что это один человек. `_ru` обновили (реальный директор), `_latin` остался со старым/сид-значением (латиница демо-директора `Никитин Сергей Викторович` из `seed.py`). H_stale.
**Фикс:** guard в `_build_director_subs` (`apply_guard_director_latin.py`) — если фамилия в `_latin` расходится с GOST-транслитом `_ru`, latin игнорируется, транслитерируется реальное ru-имя + warning. **Правило 74.**
**Урок:** зеркало-поле (`_latin`/`_initials`/`_genitive`), заполняемое отдельно от источника, дрейфует — не доверять слепо, сверять с источником + fallback на детерминированный пересчёт.

## Инцидент 52 (20-21.06.2026) — ФИО/инициалы в ES романизирует LLM по GOST вместо паспортной латиницы
**Что:** в листках «SERGEY SERGEYEVICH» (надо SERGEI), в подписи контракта «KOT IU.A.» (надо KOT Y.A.). Фамилия при этом верная (паспортная).
**Корень:** `name_substitution` подставляет паспортную латиницу, но пары строятся в исходном регистре и с ОБЫЧНЫМ пробелом, а документы рендерят ФИО ЗАГЛАВНЫМИ (`build_payslip_context.upper()`) и инициалы с НЕРАЗРЫВНЫМ пробелом (`_initials_native`/`_nbsp_initials`, U+00A0). Регистрозависимый `apply()` → пары не матчат → имя/инициал романизирует LLM. А LLM-GOST расходится с паспортом для Ю→IU/YU, Я→IA/YA, Е, Ж… (для Сергей GOST=SERGEI совпал, поэтому раньше не замечали).
**Фикс:** Pack 59 (UPPER-варианты + GOST-отчество), 59.1 (UPPER директор), 59.4 (NBSP-варианты). **Правило 75.**
**Урок:** любая трансформированная форма рендера (uppercase, NBSP, родительный падеж) требует своей пары в подстановке, иначе LLM молча перероманит имя собственное — и GOST ≠ паспорт.

## Инцидент 53 (21.06.2026) — Pack 60.0: пропатчен НЕ ТОТ эндпоинт скачивания документа
**Что:** Фаза 1 добавила `?format=` в `render_endpoints.render_single_document` (`POST …/render/{document_type}`), но фронт (`DocumentsGrid.handleDownloadOne`) качает один документ через ДРУГОЙ роут — `applications.download_single_file` (`GET …/download-file/{file_id}`). Правка работала бы, но её никто не вызывает.
**Корень:** не проследил реальный вызов фронта перед патчем — два похожих эндпоинта, фронт использует `/download-file/{id}` (он покрывает всё: docx, PDF-формы, листки, TIE).
**Фикс:** Фаза 1b (Pack 60.0b) — тот же `?format=` на боевом `download_single_file`. Патч Фазы 1 безвреден, оставлен.
**Урок:** перед патчем серверного эндпоинта — ГРЕП фронта на фактический URL (`fetch(...)`), а не догадка по имени функции.
...
**Версия документа:** 12.0 (21.06.2026 поздний вечер — **серия Pack 60.0–60.3**: скачивание любого документа и архива в 3 форматах (Word/PDF/JPEG) через новый `doc_convert.py` (docx→pdf soffice, →jpeg PyMuPDF постранично в ZIP, батч-soffice для всего архива). `?format=` на `download-file/{id}`, `render-package`, `render-package-docx/-pdf`, `translations/{kind}/download`, `translations/zip`. Фронт: 3 иконки формата на строке (DocumentsGrid + TranslationPanel), селекторы у архивов. **Инцидент 53**, **Правила 76–77**. Прежнее: 11.0 (21.06.2026 — **серия Pack 57–59.4**: пропорция неполного месяца через единый движок `prod_calendar.monthly_gross` (2-НДФЛ + листки + выписка) + anti-stale guard подписи директора + когерентность ФИО/инициалов в ES строго по паспортной латинице (UPPER- и NBSP-варианты пар + GOST-отчество) + неперенос кодов операций на ЧБ-шаблоне. Pack 58 (реалистичный отток выписки) — **откачен**, переделка в долге #12. Новые **Инциденты 51–52**, **Правила 74–75**. Серое выделение поступлений Сбера — подтверждено корректным. ⚠️ Коллизия номера Pack 57.x с cita-фичей.). Прежнее: 10.0 (10.06.2026 поздний вечер — **Pack 54 (Sber v2) FULLY DEPLOYED — серия base + fix1..fix10**. Sber v2 финализирован: подпись Кирьянова FLOATING 25мм + круглая печать FLOATING 35мм повёрнута на 25° + линия убрана + spacer 30мм + перевод RU→ES автоматом через Pack 53. `_add_floating_picture` расширен kwargs `z_order` и `rotation_deg`. **Новое Правило 73** — `_add_floating_picture` API + координаты Sber v2 + паттерн для следующих банков (ВТБ/Открытие = Pack 55.x). **fix10 критический** — layout-правки перенесены ДО проверки `mode=markers_only`). Прежнее: 9.0 (10.06.2026 вечер — Pack 53 + Pack 54 IN PROGRESS). Прежнее: 8.0 (10.06.2026 — **Pack 51 append-выписка + Pack 52 серия Ч/Б Альфа v2 + PDF + 22 fix-итерации, финал fix22**). Прежнее: 7.0 (03.06.2026 — **Pack 50.40 разбег номеров + Pack 50.41 подсветка непросмотренных документов**). Прежнее: 6.0 (26.05.2026 — **Pack 50.0 типизация + Pack 50.7 Т-9 + Pack 50.1 трудовой договор**).
