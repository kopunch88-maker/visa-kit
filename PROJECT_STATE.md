...
> **⚠️ КОЛЛИЗИЯ НОМЕРА (21.06.2026):** номер `Pack 57.x` де-факто занят зарплатной серией (пропорция неполного месяца + когерентность имён: Pack 57/57.5/59.x, уже в git). Эту cita-фичу при старте разработки **переименовать** (предлагается Pack 60.x), чтобы не путать с задеплоенной зарплатной/именной правкой.
