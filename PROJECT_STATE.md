...
> **Дата последнего обновления:** 10.06.2026 — **Pack 51 (append-выписка) + Pack 52 (Ч/Б шаблон Альфы v2 + PDF + позиционирование подписи/печатей через floating anchors)**. Pack 51: append-mode банковской выписки + удалён UI-gate (51-fix1). Pack 52: новый Ч/Б шаблон Альфа v2 + рендер DOCX→PDF через LibreOffice headless (Dockerfile + render_bank_statement_to_pdf), v2-флаг на Application, frontend swap PDF/иконка. Главное в Pack 52 — позиционирование 3 PNG (подпись Агеевой / прямоугольный штамп ДО / круглая печать «Альфа-Банк») на маркеры `__STAMP_*__` в шаблоне. Прошли через 22 итерации (fix1..fix22), ключевое озарение fix17 — **прод-LibreOffice НЕ клипует floating-anchor** (моё локальное превью клипует) → большие отрицательные Y буквально улетали на 200мм вверх. Решение: ВСЕ 3 картинки якорятся к параграфу прямоугольной печати (R0C2, inline = ровно на линии), у подписи и круглой маленькие y_off (±2..5мм). Финальные значения после визуального сравнения с реальной выпиской на проде: signature x=-60 y=+2, employee inline, bank x=+80 y=-5. Предыдущее: 03.06.2026 — **Pack 50.40 + 50.41**: разбег реестровых номеров по дате выдачи + подсветка непросмотренных документов. Предыдущее: 30.05.2026 — **Pack 50.32–50.39**: СТД-Р правка юриста + ФИЧА «автозаполнение заявки из текста менеджера». Предыдущее: 29.05.2026 — **Pack 50.12–50.31 (большая NAIM-сессия)**. Предыдущее: 26.05.2026 Pack 41.0-H..Q + 50.0/50.7/50.1 + 47/48/49.
...
### Pack 53.x — Шаблоны для ВТБ (BIK 044525187) и Открытие (BIK 044525985) — TODO

⚠️ **Перенумеровано из Pack 50.x → 51.x → 53.x:** номер 50 ушёл на NAIM-линию, 51 — на append-mode выписки, 52 — на Ч/Б шаблон Альфы v2. При возврате к выпискам ВТБ/Открытие — нумеровать как 53.x.
...
**Версия документа:** 8.0 (10.06.2026 — **Pack 51 append-выписка + Pack 52 серия Ч/Б Альфа v2 + PDF рендеринг + позиционирование подписи/печатей через floating anchors (22 fix-итерации, финал fix22)**). Добавлены: TL;DR сессии 09-10.06.2026, новый шаблон `bank_statement_template_v2.docx` + ассеты `v2/{signature,stamp_employee,stamp_bank}.png`, эндпоинт `render_bank_statement_to_pdf()` через `soffice --headless`, Dockerfile с libreoffice, флаг `Application.bank_template_legacy_v1`, **Правило 72** (floating-anchors: прод ≠ локальный LibreOffice, якорить к inline-параграфу), **Инцидент 50** (Pack 52 серия: клипинг локально → улёт картинок на проде). Перенумеровано: Pack 53.x = выписки ВТБ/Открытие (было 51.x). Прежнее: 7.0 (03.06.2026 — **Pack 50.40 разбег номеров + Pack 50.41 подсветка непросмотренных документов (+fix1 рамка)**). Прежнее: 6.0 (26.05.2026 — **Pack 50.0 типизация заявок + Pack 50.7 приказ Т-9 + Pack 50.1 серия трудового договора (A-G, финал 0f3d684)**).

**Базируется на:** 7.0 (03.06.2026 — Pack 50.40 + 50.41) ← 6.0 (26.05.2026 — Pack 50.0/50.7/50.1 трудовой договор) ← 5.0 (25.05.2026 вечер — Pack 47/48/49 банковские выписки Сбер/ТБанк) ← 4.4 (24.05.2026 вечер — Pack 46.0 Диплом для хурадо) ← 4.3 (Pack 43.0+44.0+45.0) ← 4.2 (Pack 40-42) ← 4.1 (Pack 39.0) ← 4.0 (Pack 37.x).
