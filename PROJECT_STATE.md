...
> **Дата последнего обновления:** 03.06.2026 — **Pack 50.40 + 50.41**: разбег реестровых номеров по дате выдачи (КНД 1122035 ~+500/раб.день, апостили НПД+СФР ~+350/раб.день, строго монотонно по дате — единый `_doc_numbering.py`) + **подсветка непросмотренных документов** (сетки сгенерированных + сканы клиента: таблица `document_view_state`, 3 эндпоинта doc-view-state, точка-toggle, снятие на open/download/ZIP, синяя рамка непросмотренным — Pack 50.41-fix1). Предыдущее: 30.05.2026 — **Pack 50.32–50.39**: СТД-Р правка юриста (50.37) + запрет остаточной кириллицы в переводе (50.36) + **ФИЧА «автозаполнение заявки из текста менеджера»** (50.38 серия: город подачи A1-A3 + парсер B + fuzzy-резолвер D1 + применение D3-1/D3-2 + фронт D4 + фиксы D-fix/D1-fix) + город подписания из юр.адреса (50.39). Предыдущее: 29.05.2026 — **Pack 50.12–50.31 (большая NAIM-сессия)**: документы найма СФР (СОО доводка, апостиль Минфина/СФР, скрытие RETA) + телефон РФ + даты командировки + ОКЗ-код для СТД-Р (поле + LLM + заполнение 68 должностей) + полная вёрстка СТД-Р под эталон Орлова (разрыв страниц, нумерация, шрифт 5.5pt, ПРИКАЗ, центрирование, удаление пустых строк, ширины колонок, воздух в шапке) + выписка для найма (аванс+зарплата по трудовому договору, −13% НДФЛ). Предыдущее: 26.05.2026 Pack 41.0-H..Q + 50.0/50.7/50.1 + 47/48/49.
...
### Pack 51.x — Шаблоны для ВТБ (BIK 044525187) и Открытие (BIK 044525985) — TODO

⚠️ **Перенумеровано из Pack 50.x в Pack 51.x:** номер Pack 50 ушёл на NAIM-линию (см. TL;DR Pack 50.0/50.7/50.1 серии). При возврате к выпискам — нумеровать как 51.x.
...
**Версия документа:** 7.0 (03.06.2026 — **Pack 50.40 разбег номеров + Pack 50.41 подсветка непросмотренных документов (+fix1 рамка)**). Добавлены: TL;DR сессии 03.06.2026, миграция `document_view_state`, единый `_doc_numbering.py`. Прежнее: 6.0 (26.05.2026 — **Pack 50.0 типизация заявок + Pack 50.7 приказ Т-9 + Pack 50.1 серия трудового договора (A-G, финал 0f3d684)**). Добавлены: TL;DR блоки Pack 50.0/50.7/50.1 серий, перенумерован Pack 50.x→51.x в Roadmap (выписки ВТБ/Открытие), новый раздел Pack 50.X в Roadmap (следующие шаблоны NAIM), Инцидент 49 (Pack 50.1-H str.replace × 3 с одним якорем).

**Базируется на:** 6.0 (26.05.2026 — Pack 50.0/50.7/50.1 трудовой договор) ← 5.0 (25.05.2026 вечер — Pack 47/48/49 банковские выписки Сбер/ТБанк) ← 4.4 (24.05.2026 вечер — Pack 46.0 Диплом для хурадо) ← 4.3 (Pack 43.0+44.0+45.0) ← 4.2 (Pack 40-42) ← 4.1 (Pack 39.0) ← 4.0 (Pack 37.x).
