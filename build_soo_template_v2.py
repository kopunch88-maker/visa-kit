r"""
Pack 50.12-C-r2 — Генератор soo_template.docx (Свидетельство об отъезде).

Изменения r2 (по замечаниям Кости, сравнение с эталоном Регины):
  - Убрана серая заливка плашек заголовков
  - Заголовки разделов = обычные параграфы жирным с отступом (не таблицы-плашки)
  - Увеличен межстрочный интервал (line_spacing 1.15) и интервалы между
    параграфами — текст размещён "воздушнее" как в эталоне
"""
...
def _section_header(doc, num_text, title_text):
    """Заголовок раздела: обычный параграф жирным с отступом слева.
    БЕЗ рамки и БЕЗ заливки (как в эталоне Регины)."""
    p = doc.add_paragraph()
