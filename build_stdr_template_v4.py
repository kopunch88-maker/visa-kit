r"""
Pack 50.9-C-r4 — Генератор шаблона stdr_template.docx по эталону СФР.

Изменения r4:
  - Убран лишний заголовок "Сведения о трудовой деятельности..." перед Таблицей 1
    (его нет в эталоне — он уже в шапке таблицы как gridSpan=8).
  - Строки "Подано заявление о..." сделаны как параграфы с tab + underline на дате
    (как в эталоне, не таблицей).
  - Линии под датами через w:u (underline) — единый стиль с эталоном.
...
def main() -> int:
    print(f"Building STDR template v4 → {OUTPUT_PATH}")
    ...
    if OUTPUT_PATH.exists():
        backup = OUTPUT_PATH.with_suffix(
            f".docx.bak_pre_pack50_9_C_r4_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
